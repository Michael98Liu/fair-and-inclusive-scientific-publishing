...
class GetPub:
    def __init__(self, L, M, outcome, DIR='/scratch/fl1092/capstone/'):
        self.L = L
        self.M = M
        self.LOG = self.L.LOG
        self.debug = self.L.debug
        self.dir = DIR+'matching/'+outcome+'/'
    ...
    def getSeedBadness(self):
        ...
        self.n_count.to_csv(self.dir+'NormalCount.csv', sep='\t', index=False)
        self.q_count.to_csv(self.dir+'QuestionableCount.csv', sep='\t', index=False)
        self.s_count.to_csv(self.dir+'SuspiciousCount.csv', sep='\t', index=False)

    def getBadPetter(self, Aq=2/3, Bq=2, As=2, Bs=2):
    ...
    def load_lines(self):
        cols = ['EditorsNewId', 'issn', 'NewAuthorId', 'EditorYear', 'EdiCount', 'AutCount', 'seed']
        types = {x: (int if x!='issn' else str) for x in cols}

        self.n_count = pd.read_csv(self.dir+'NormalCount.csv', sep='\t', usecols=cols, dtype=types)
        self.q_count = pd.read_csv(self.dir+'QuestionableCount.csv', sep='\t', usecols=cols, dtype=types)
        self.s_count = pd.read_csv(self.dir+'SuspiciousCount.csv', sep='\t', usecols=cols, dtype=types)
